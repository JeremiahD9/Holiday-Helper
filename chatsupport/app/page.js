...
export default function Home() {
  ...
  const calculateProgress = () => {
    if(vacationExpenses.vacation_budget.total_budget === null || moneySpent === 0)
    {
      setProgress(0)
    }
    else
    {
      let percent = ((moneySpent/vacationExpenses.vacation_budget.total_budget) * 100)
      if(percent < 50)
      {
  ...
  const updateDescription = async () => {
    const response = await fetch("api/description", {
      ...
      body: JSON.stringify({conversation: [...messages, {role: "user", content: message}], expenseJson: vacationExpenses})
    })
        ...
        <Smartphone sx={{marginLeft: 1, width: "30%"}}>
          <Stack direction="column" width="100%" height="100%" p={2} spacing={3}>
            ...
            <Stack direction="row" spacing={2}>
              ...
              <Button variant="contained" onClick={() => {sendMessage(); updateExpenses(); updateDescription();}}>
                Send
              </Button>
            </Stack>
          </Stack>
        </Smartphone>
        <Box sx={{display: "flex", flexDirection: "column", height: "100%", width: "70%", justifyContent: "center", alignItems: "center"}}>
          <Card sx={{height: "100%"}} width="90%">
            <Stack sx={{height: "75px", marginTop: 2, width: "100%", display: "flex", justifyContent: "center", alignItems: "center"}}>
              
              
              Budget: ${vacationExpenses.vacation_budget?.total_budget || "N/A"}
              <ProgressBar bgcolor={progressColor} completed={progress} fullWidth/>
            </Stack>
            <Box width="100%" height= "100%" display="flex" flexDirection="row">
              <Box width="100%" height="100%">
                <Grid container spacing={2} rowSpacing={5}>
                  <Grid xs={4}>
                    <Item>Travel: ${vacationExpenses.vacation_budget.expenses?.flight || "N/A"} <br/> {expensesDescription.vacation_budget.expenses?.flight || "No Description Available"}</Item>
                  </Grid>
                  <Grid xs={4}>
                    <Item>Accommodations: ${vacationExpenses.vacation_budget.expenses?.accommodation || "N/A"} <br/> {expensesDescription.vacation_budget.expenses?.accommodation || "No Description Available"}</Item>
                  </Grid>
                  <Grid xs={4}>
                    <Item>Activities: ${vacationExpenses.vacation_budget.expenses?.activities || "N/A"} <br/> {expensesDescription.vacation_budget.expenses?.activities || "No Description Available"}</Item>
                  </Grid>

                  <Grid xs={4}>
                    <Item>Food: ${vacationExpenses.vacation_budget.expenses?.food || "N/A"} <br/> {expensesDescription.vacation_budget.expenses?.food || "No Description Available"}</Item>
                  </Grid>
                  <Grid xs={4}>
                    <Item>Transportation: ${vacationExpenses.vacation_budget.expenses?.transportation || "N/A"} <br/> {expensesDescription.vacation_budget.expenses?.transportation || "No Description Available"}</Item>
                  </Grid>
                  <Grid xs={4}>
                    <Item>Souvenirs: ${vacationExpenses.vacation_budget.expenses?.souvenirs || "N/A"} <br/> {expensesDescription.vacation_budget.expenses?.souvenirs || "No Description Available"}</Item>
                  </Grid>
                </Grid>
              </Box>
